...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 1.0 && time <= 2.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >= 5.0 && time <= 6.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
               if (time >= 11.0 && time <= 12.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '4':
                ...
                break;
            case '5':
                if (time >= 19.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
